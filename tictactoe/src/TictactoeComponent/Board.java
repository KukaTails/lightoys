...
public class Board {
    ...
    public static void main(String[] args)
    {
        ...
        while ((!board.IsWin(board.AI) && !board.IsWin(board.MAN))
            ...
            PrintBoard(board);
            if (board.CanMove(board.MAN) || board.CanMove(board.AI)) {
                ...
                board.record[aiBestMove.get() / 3][aiBestMove.get() % 3] = board.AI;
            }
            ...
            if (board.CanMove(board.MAN) || board.CanMove(board.AI)) {
                ...
                board.record[manBestMove.get() / 3][manBestMove.get() % 3] = board.MAN;
            }
        }
    ...
    public static void PrintBoard(Board board)
    {
        for (int i = 0; i < 3; ++ i) {
            for (int j = 0; j < 3; ++ j) {
                if (board.record[i][j] == board.AI)
                    System.out.printf("%4s", "A");
                else if (board.record[i][j] == board.MAN)
    ...
    private int Eval()
    {
       ...
       //if (IsWin(MAN)) return Integer.MIN_VALUE + 10;
        ...
        for (int i = 0; i < 3; ++ i)
            value += ((tmp_record[i][0] + tmp_record[i][1] + tmp_record[i][2]) / 3 == AI) ? 3 : 0;
        for (int j = 0; j < 3; ++ j)
            value += ((tmp_record[0][j] + tmp_record[1][j] + tmp_record[2][j]) / 3 == AI) ? 3 : 0;
        value += ((tmp_record[0][0] + tmp_record[1][1] + tmp_record[2][2]) / 3 == AI) ? 3 : 0;
        value += ((tmp_record[2][0] + tmp_record[1][1] + tmp_record[0][2]) / 3 == AI) ? 3 : 0;
        ...
        for (int i = 0; i < 3; ++ i)
            value += ((tmp_record[i][0] + tmp_record[i][1] + tmp_record[i][2]) / 3 == MAN) ? -3 : 0;
        for (int j = 0; j < 3; ++ j)
            value += ((tmp_record[0][j] + tmp_record[1][j] + tmp_record[2][j]) / 3 == MAN) ? -3 : 0;
        value += ((tmp_record[0][0] + tmp_record[1][1] + tmp_record[2][2]) / 3 == MAN) ? -3 : 0;
        value += ((tmp_record[2][0] + tmp_record[1][1] + tmp_record[0][2]) / 3 == MAN) ? -3 : 0;
